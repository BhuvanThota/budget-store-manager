...
// The component now needs to know about the user to display name/image
...
export default function MobileMenu({ user }: MobileMenuProps) {
  ...
  // Helper component for menu items to keep the code clean
  const MenuItem = ({ href, children, onClick }: { href?: string; children: React.ReactNode; onClick?: () => void }) => {
    const content = (
      <div
        className="flex items-center w-full px-3 py-2 text-sm text-gray-300 rounded-md hover:bg-gray-700 hover:text-white transition-colors"
        onClick={() => {
          if (onClick) onClick();
          setIsOpen(false);
        }}
    ...
    return href ? <Link href={href}>{content}</Link> : <button className="w-full">{content}</button>;
  };
  ...
  return (
    <div className="md:hidden relative" ref={menuRef}>
      ...
      <button
        ...
        className="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white rounded-full"
      ...
      {isOpen && (
        <div
          className="absolute top-full right-0 mt-2 w-72 bg-gray-900 text-gray-200 rounded-lg border border-gray-700 shadow-lg z-20"
        >
          {/* Header */}
